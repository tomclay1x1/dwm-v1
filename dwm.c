...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMWindowTypeDialog, NetClientList, NetLast }; /* EWMH atoms hello */
...
drawbar(Monitor *m)
{
    ...
    if (m == selmon) { /* status is only drawn on selected monitor */
        ...
        tw = TEXTW(stext); 
        drw_text(drw, m->ww - tw - 2 * sp, 0, tw, bh, lrpad / 2, stext, 0); /* Patched */
    }
    ...
    int archw = TEXTW(" "); 
    drw_setscheme(drw, scheme[SchemeNorm]);
    x = drw_text(drw, 0, 0, archw, bh, lrpad / 2, " ", 0);
    /* ---------------------------- */
    ...
    if ((w = m->ww - tw - x) > bh) {
        if (m->sel) {
            ...
            drw_text(drw, x, 0, w - 2 * sp, bh, lrpad / 2, m->sel->name, 0); /* Patched */
            if (m->sel->isfloating)
...
horizgrid(Monitor *m) {
    ...
    for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next), i++) {
        if (i < ntop) {
            resize(c, mx, my, (mw / ntop) + (i < mrest ? 1 : 0) - (2*c->bw), mh - (2*c->bw), 0);
            mx += WIDTH(c) + iv;
        } else {
